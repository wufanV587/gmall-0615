...
public class SkuBoundsServiceImpl extends ServiceImpl<SkuBoundsDao, SkuBoundsEntity> implements SkuBoundsService {
    ...
    public List<ItemSaleVO> queryItemSaleVOs(Long skuId) {
        ...
        //查询积分信息
        List<SkuBoundsEntity> skuBoundsEntitylist = this.list(new QueryWrapper<SkuBoundsEntity>().eq("sku_id", skuId));
        if (!CollectionUtils.isEmpty(skuBoundsEntitylist)){
            ItemSaleVO saleVO = new ItemSaleVO();
            saleVO.setType("积分");
            BigDecimal buyBounds = skuBoundsEntitylist.get(0).getBuyBounds();
            BigDecimal growBounds = skuBoundsEntitylist.get(0).getGrowBounds();
